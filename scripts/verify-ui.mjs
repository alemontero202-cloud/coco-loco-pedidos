...
const selectors = [...js.matchAll(/\$\(['"]#([^'"]+)["']\)/g)].map(m => m[1]).filter(selector => !/\s/.test(selector));
const missing = [...new Set(selectors)].filter(id => !ids.has(id));
if (missing.length) {
  console.error(`Selectores ID sin elemento HTML: ${missing.join(', ')}`);
  process.exit(1);
}
...
console.log(`UI selector check OK (${selectors.length} selectores ID comprobados).`);
