...
function renderHistory() { const visible = orders.filter(o => history === 'all' ? true : history === 'pending' ? !['delivered', 'cancelled'].includes(o.status) : o.status === history); $('#history-list').innerHTML = visible.length ? visible.map(o => card(o)).join('') : '<p class="empty">No hay pedidos para mostrar.</p>'; }
